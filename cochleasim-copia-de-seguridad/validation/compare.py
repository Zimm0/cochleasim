...
class ComparisonResult:
    ...
    reference : object
        A ReferenceCurve or ReferencePoints (cochleasim.validation.reference).
    metric : str
        "percent" -> 100*(model-ref)/ref
        "absolute" -> model-ref
    interpolated : dict[str, ndarray]
        Each model curve's y, resampled onto reference.x. Populated by
        compare_curves.
    error : dict[str, ndarray]
        Per-point error of each model curve against reference.y, in the
        chosen metric. Populated by compare_curves.
    """
    model: dict
    reference: object
    metric: str
    ...
    def goodness_of_fit(self, log=False):
        ...
        log : bool
            ...
            quantities that are roughly exponential/multiplicative, e.g.
            CF or Q10dB -- a fixed % error corresponds to a fixed
            absolute log-error, unlike in linear space). Points where
            either series is <= 0 are dropped when log=True.
    ...
    def plot(self, ax=None, xlabel="x", ylabel="y", y_scale=1.0):
        ...
        y_scale : float
            ...
            as kHz). Purely a display-time conversion -- summary_table(),
            goodness_of_fit() and self.error remain in the original units.
        """
...
def compare_curves(model, reference, metric="percent"):
    ...
    model : dict[str, tuple[array_like, array_like]] or tuple[array_like, array_like]
        Either {label: (x, y)} for multiple curves, or a single (x, y)
        tuple (treated as one curve labeled "model").
    reference : ReferenceCurve or ReferencePoints
    metric : str
        "percent" or "absolute" (see ComparisonResult).
