...
class ReferenceCurve:
    ...
    y : ndarray
    label : str
        Short name for legends (e.g. "Greenwood (1990), human").
    source : str
        Full citation.
    """
    x: np.ndarray
...
def greenwood_frequency(x_from_apex, A, a, k):
    ...
    Parameters
    ----------
    x_from_apex : array_like
        Fractional distance along the cochlea measured FROM THE APEX
        (0 = apex, 1 = base) -- Greenwood's own convention, the opposite
        of solve_pressure's x (measured from the stapes/base). Use
        x_from_apex_fraction() to convert model positions before calling
        this.
    A, a, k : float
...
def x_from_apex_fraction(x_cm, L_cm):
    """
    Convert the model's base-referenced position (x_cm, 0=stapes) into
    Greenwood's apex-referenced fractional position (0=apex, 1=base).

    Parameters
    ----------
    x_cm : array_like
    L_cm : float
        Total cochlear length (params.L).
...
def load_greenwood_curve(species, n_points=500):
    ...
    ----------
    species : str
        Row key in data/reference/greenwood_coefficients.csv (e.g. "human", "cat").
    n_points : int
        Number of points to sample the curve at.

    Returns
    -------
    ReferenceCurve
        x : fractional position from the base (0=stapes, 1=apex), matching
            solve_pressure's own convention divided by L.
...
def load_erb_q_curve(cf_range_hz=(150.0, 16000.0), n_points=200):
    ...
    reference (Moore & Glasberg 1983), over a range of CFs.

    Analytic -- no digitization involved, reuses
    cochleasim.models.auditory_filterbank.erb_hz().

    Parameters
    ----------
    cf_range_hz : tuple(float, float)
    n_points : int
...
def load_gain_reference(species):
    ...
    ----------
    species : str
        Value in the 'species' column of data/reference/neely_kim_1986_gain.csv.
...
def load_phase_accumulation_reference(species):
    ...
    ----------
    species : str
        Value in the 'species' column of
